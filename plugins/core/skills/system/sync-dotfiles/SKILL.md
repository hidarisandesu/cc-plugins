...
- **差分がある場合**: 差分内容を報告し、AskUserQuestion で「このPCに適用しますか？」と確認する。選択肢は「適用する」「スキップ」を用意する。
  - 「適用する」: `chezmoi apply -v --force` を実行し、結果を報告する。（`--force` は対話プロンプトを回避するため。確認はAskUserQuestionで取得済み）
  - 「スキップ」: 適用せずに完了報告へ進む。
...
- **ローカル適用確認**: chezmoi diffの結果。差分ありの場合AskUserQuestionで確認
