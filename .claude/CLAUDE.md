...
## なぜ hook-pretool-reply の要求先をターン末尾にするか
ツール間に書いた本文が、表示されない思考チャネル（thinking）に落ちる現象がある
（2026-08-19 / Linux 側マシンのセッション記録で確認。発生は不安定）。
このため「ツールの前に本文を一言書け」という当初の要求は確実には満たせず、
モデル自身は書いたと認識するため注入文による自己修正も期待できない。
表示が保証されるのはターンの先頭と末尾だけなので、要求は末尾への再掲に置く。
（先頭はすでに hook-reply-first が担っている。）
